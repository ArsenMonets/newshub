...
## 🧩 Застосування архітектурних патернів GoF (Gang of Four)

У серверній частині платформи чітко прослідковується реалізація фундаментальних шаблонів проєктування GoF, що забезпечує низьку зв'язність (Loose Coupling) та високу гнучкість коду.

### 1. Патерн "Декоратор" (Decorator / Wrapper)

**Призначення:** Динамічне розширення функціональності об'єкта без зміни його базової структури та використання успадкування.

**Реалізація в проєкті:** Клас `CustomUserDetails` виступає декоратором для сутності користувача `UserEntity`. Фреймворк Spring Security вимагає для автентифікації інтерфейс `UserDetails`, який містить методи перевірки стану акаунту (`isEnabled()`, `isAccountNonLocked()`). Замість того, щоб забруднювати чисту доменну модель `UserEntity` специфічними методами безпеки або створювати складні ієрархії наслідування, ми "обгортаємо" її в декоратор.

* **Код:** `CustomUserDetails` приймає в конструктор `UserEntity` і делегує йому стандартні виклики (наприклад, `getUsername()` повертає `user.getLogin()`), одночасно додаючи нову логіку: конвертацію внутрішньої ролі у формат `SimpleGrantedAuthority` з префіксом `"ROLE_"` та динамічну перевірку блокування (`!user.isBlocked()`).
...
### 2. Патерн "Спостерігач" (Observer / Publisher-Subscriber)

**Призначення:** Створення механізму підписки, який дозволяє одним об'єктам стежити за подіями в інших об'єктах та реагувати на них у реальному часі.

**Реалізація в проєкті:** Подієво-орієнтована шина WebSocket реалізує класичну поведінку цього патерну. Роль **Суб'єкта (Видавця)** виконує `NewsService`. Роль **Спостерігачів (Підписників)** виконують Angular-клієнти, які підписуються на відповідні STOMP-топіки брокера повідомлень. `NewsWebSocketController` виступає як медіатор/диспетчер подій через `SimpMessagingTemplate`.

* **Логіка роботи:** Коли в `NewsService` відбуваються мутації даних (`create`, `update`, `delete`), сервіс публікує подію, викликаючи методи контролера (наприклад, `sendNewsCreate()`). Контролер асинхронно транслює оновлений стан (об'єкт `NewsPreviewDTO` або `id`) усім підключеним клієнтам через канали `/topic/news/created`, `/topic/news/updated` тощо, автоматично синхронізуючи інтерфейс користувачів без перезавантаження сторінок.

```mermaid
graph TD
    Publisher[NewsService <br> Видавець події] -->|Виклик тригера| Broker[NewsWebSocketController <br> Диспетчер / SimpMessagingTemplate]
    
    Broker -->|Трансляція /topic/news/*| Sub1[Angular Client 1 <br> Спостерігач]
    Broker -->|Трансляція /topic/news/*| Sub2[Angular Client 2 <br> Спостерігач]
    Broker -->|Трансляція /topic/news/*| Sub3[Angular Client N <br> Спостерігач]
